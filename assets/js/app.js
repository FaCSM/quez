...
modalRegion.forEach(item => {
    ...
    startButton.addEventListener('click', () => {
        ...
        if (item.classList.contains('brest')) {
            makeInfo('Брест', 'brest.png', arr.length)
        }
        if (item.classList.contains('grodno')) {
            makeInfo('Гродно', 'grodno.png', arr.length)
        }
        if (item.classList.contains('gomel')) {
            makeInfo('Гомель', 'gomel.png', arr.length)
        }
        if (item.classList.contains('vitebsk')) {
            makeInfo('Витебск', 'vitebsk.png', arr.length)
        }
        if (item.classList.contains('minsk')) {
            makeInfo('Минск', 'minsk.png', arr.length)
        }
        if (item.classList.contains('mogilev')) {
            makeInfo('Могилев', 'mogilev.png', arr.length)
        }
...
function makeInfo(townName, photoName, arrLength) {
    ...
    const infoInnerH3 = infoInner.querySelector('.titles > .h3')
    ...
    infoInnerImg.setAttribute('src', `assets/images/info/${photoName}`)
    trueBox.innerHTML = ''
